# Summary 

## [DevOps](../DevOps/README.md) 

- [Introduction](../DevOps/README.md) 

- [发布与部署](../DevOps/发布与部署/README.md) 
  - [Docker 容器化部署](../DevOps/发布与部署/Docker 容器化部署.md) 
  - [K8s 部署](../DevOps/发布与部署/K8s 部署.md) 
- [日志跟踪](../DevOps/日志跟踪/README.md) 
  - [Log4j2](../DevOps/日志跟踪/Log4j2.md) 
  - [Logback](../DevOps/日志跟踪/Logback.md) 
  - [Slf4j](../DevOps/日志跟踪/Slf4j.md) 
  - [分布式链路跟踪](../DevOps/日志跟踪/分布式链路跟踪.md) 
  - [应用监控](../DevOps/日志跟踪/应用监控.md) 
  - [日志处理](../DevOps/日志跟踪/日志处理.md) 
- [测试](../DevOps/测试/README.md) 
  - [JUnit 4](../DevOps/测试/JUnit 4.md) 
  - [JUnit 5](../DevOps/测试/JUnit 5.md) 
  - [Mockito](../DevOps/测试/Mockito.md) 
  - [单元测试](../DevOps/测试/单元测试.md) 
  - [集成测试](../DevOps/测试/集成测试.md) 

## [Java Web](../Java Web/README.md) 

- [Introduction](../Java Web/README.md) 

- [Jetty](../Java Web/Jetty/README.md) 
- [Servlet](../Java Web/Servlet/README.md) 
- [Tomcat](../Java Web/Tomcat/README.md) 

## [Web 应用](../Web 应用/README.md) 

- [Introduction](../Web 应用/README.md) 

- [WebFlux](../Web 应用/WebFlux/README.md) 
- [依赖声明与注入](../Web 应用/依赖声明与注入/README.md) 
  - [Bean 声明](../Web 应用/依赖声明与注入/Bean 声明.md) 
  - [Bean 注入](../Web 应用/依赖声明与注入/Bean 注入.md) 
  - [Bean 生命周期](../Web 应用/依赖声明与注入/Bean 生命周期.md) 
- [应用配置](../Web 应用/应用配置/README.md) 
  - [上下文配置](../Web 应用/应用配置/上下文配置/README.md) 
    - [外部参数](../Web 应用/应用配置/上下文配置/外部参数.md) 
    - [生命周期](../Web 应用/应用配置/上下文配置/生命周期.md) 
    - [自动配置](../Web 应用/应用配置/上下文配置/自动配置.md) 
  - [参数注入](../Web 应用/应用配置/参数注入/README.md) 
- [视图与模板](../Web 应用/视图与模板/README.md) 
  - [模板渲染](../Web 应用/视图与模板/模板渲染.md) 
  - [静态资源](../Web 应用/视图与模板/静态资源.md) 
- [请求处理](../Web 应用/请求处理/README.md) 
  - [WebSocket](../Web 应用/请求处理/WebSocket.md) 
  - [异步请求](../Web 应用/请求处理/异步请求.md) 
  - [请求响应](../Web 应用/请求处理/请求响应.md) 
  - [路由与参数](../Web 应用/请求处理/路由与参数.md) 
- [过滤器](../Web 应用/过滤器/README.md) 
  - [内置过滤器](../Web 应用/过滤器/内置过滤器.md) 

## [winter-boot](../winter-boot/README.md) 

- [Introduction](../winter-boot/README.md) 


## [安全认证](../安全认证/README.md) 

- [Introduction](../安全认证/README.md) 

- [Pac4j](../安全认证/Pac4j/README.md) 
- [Shiro](../安全认证/Shiro/README.md) 
- [Spring Security](../安全认证/Spring Security/README.md) 
  - [CORS](../安全认证/Spring Security/CORS.md) 
  - [CSRF](../安全认证/Spring Security/CSRF.md) 
  - [JWT](../安全认证/Spring Security/JWT.md) 
  - [表单登录](../安全认证/Spring Security/表单登录.md) 
  - [验证码](../安全认证/Spring Security/验证码.md) 

## [导论](../导论/README.md) 

- [Introduction](../导论/README.md) 

- [DevTools](../导论/DevTools.md) 
- [快速开始](../导论/快速开始.md) 
- [框架对比](../导论/框架对比.md) 
- [生态圈项目](../导论/生态圈项目/README.md) 
  - [Spring Integration](../导论/生态圈项目/Spring Integration.md) 
  - [Spring Messaging](../导论/生态圈项目/Spring Messaging.md) 

## [微服务](../微服务/README.md) 

- [Introduction](../微服务/README.md) 

- [Alibaba Cloud](../微服务/Alibaba Cloud/README.md) 
- [DDD](../微服务/DDD/README.md) 
  - [POJO](../微服务/DDD/POJO.md) 
- [Spring Cloud](../微服务/Spring Cloud/README.md) 

## [数据访问](../数据访问/README.md) 

- [Introduction](../数据访问/README.md) 

- [Hibernate](../数据访问/Hibernate/README.md) 
- [JDBC](../数据访问/JDBC/README.md) 
  - [JdbcTemplate](../数据访问/JDBC/JdbcTemplate.md) 
  - [数据源](../数据访问/JDBC/数据源.md) 
  - [高性能 Java 持久化](../数据访问/JDBC/高性能 Java 持久化.md) 
  - [Druid](../数据访问/JDBC/Druid/README.md) 
  - [Hibernate](../数据访问/JDBC/Hibernate/README.md) 
  - [HikariCP](../数据访问/JDBC/HikariCP/README.md) 
    - [HikariCP](../数据访问/JDBC/HikariCP/HikariCP.md) 
  - [JPA](../数据访问/JDBC/JPA/README.md) 
    - [JPA](../数据访问/JDBC/JPA/JPA.md) 
  - [事务](../数据访问/JDBC/事务/README.md) 
    - [事务处理](../数据访问/JDBC/事务/事务处理.md) 
- [MyBatis](../数据访问/MyBatis/README.md) 
  - [Mybatis Generator](../数据访问/MyBatis/Mybatis Generator.md) 
  - [Mybatis 插件](../数据访问/MyBatis/Mybatis 插件.md) 
  - [SpringBoot 集成](../数据访问/MyBatis/SpringBoot 集成.md) 
  - [XML 映射](../数据访问/MyBatis/XML 映射.md) 
  - [上下文配置](../数据访问/MyBatis/上下文配置.md) 
  - [动态 SQL](../数据访问/MyBatis/动态 SQL.md) 
  - [注解映射](../数据访问/MyBatis/注解映射.md) 
- [Redis](../数据访问/Redis/README.md) 
- [数据库连接池](../数据访问/数据库连接池/README.md) 
- [缓存](../数据访问/缓存/README.md) 
  - [应用缓存](../数据访问/缓存/应用缓存.md) 
  - [Hazelcast](../数据访问/缓存/Hazelcast/README.md) 

## [架构机制](../架构机制/README.md) 

- [Introduction](../架构机制/README.md) 

- [依赖注入](../架构机制/依赖注入/README.md) 
  - [事务源码分析](../架构机制/依赖注入/事务源码分析.md) 
  - [循环依赖](../架构机制/依赖注入/循环依赖.md) 
- [切面编程](../架构机制/切面编程/README.md) 
  - [AOP](../架构机制/切面编程/AOP.md) 
- [加载与启动](../架构机制/加载与启动/README.md) 
  - [Jar 加载](../架构机制/加载与启动/Jar 加载.md) 
  - [启动流程](../架构机制/加载与启动/启动流程.md) 
- [请求处理](../架构机制/请求处理/README.md) 
  - [请求的处理流程](../架构机制/请求处理/请求的处理流程.md) 

## [阿里云](../阿里云/README.md) 

- [Introduction](../阿里云/README.md) 

- [OSS](../阿里云/OSS.md) 

## [集成调度](../集成调度/README.md) 

- [Introduction](../集成调度/README.md) 

- [定时任务](../集成调度/定时任务/README.md) 
  - [TaskScheduler](../集成调度/定时任务/TaskScheduler.md) 
  - [定时任务](../集成调度/定时任务/定时任务.md) 
  - [定时器](../集成调度/定时任务/定时器.md) 
- [消息通知](../集成调度/消息通知/README.md) 
  - [微信](../集成调度/消息通知/微信.md) 
  - [短信](../集成调度/消息通知/短信.md) 
  - [邮件](../集成调度/消息通知/邮件.md) 
  - [钉钉](../集成调度/消息通知/钉钉.md) 
